...
现有 abtop 以 TUI 方式提供了很强的本地观测能力，但它更适合终端内全屏查看；观察者要把这些能力重构成一个常驻菜单栏、可通知、可远程同步的桌面端监控中心。

### Proposed Solution

观察者提供一个 CleanMyMac 风格的 macOS 菜单栏面板：默认展示 Agent 健康总览、活跃 session 列表和关键风险提示；点击 session 进入详情视图，展示上下文、token、工具调用、文件访问、子进程、端口、错误和限流信息。
